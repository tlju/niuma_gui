...
<tr><td>堡垒机连接</td><td>通过堡垒机交互式菜单连接到目标资产（需堡垒机已连接且资产已配置认证信息）</td></tr>
...
<h3>堡垒机连接流程</h3>
<p>系统通过堡垒机连接目标服务器采用交互式菜单操作方式：</p>
<ol>
<li><b>连接堡垒机</b> — 系统启动时自动使用配置的堡垒机账号密码进行SSH连接</li>
<li><b>二次认证（如需要）</b> — 连接成功后，堡垒机可能输出"2nd Password:"提示
    <ul>
    <li>弹出二次认证对话框，输入6位数字动态口令（OTP）</li>
    <li>输入满6位后自动提交</li>
    <li>如果口令错误，会重新弹出对话框提示重新输入</li>
    <li>最多允许5次重试，超过后需重新连接堡垒机</li>
    </ul>
</li>
<li><b>进入堡垒机菜单</b> — 认证成功后，堡垒机显示交互式菜单界面，弹出服务器选择对话框（非模态，不阻塞主界面）</li>
</ol>

<h3>操作步骤 — 通过堡垒机连接资产</h3>
<ol>
<li>确保堡垒机已成功连接（状态栏显示绿色"已连接"）</li>
<li>如需二次认证，按上述流程完成动态口令验证</li>
<li>在资产列表中，有网络地址且配置了用户名和密码的资产行会出现"连接"按钮</li>
<li>点击"连接"按钮，系统将：
    <ul>
    <li>在堡垒机菜单中搜索目标资产IP地址</li>
    <li>自动选择匹配的资产</li>
    <li>使用资产表中存储的用户名和密码进行登录认证</li>
    </ul>
</li>
<li>连接成功后，状态栏会显示当前连接的资产IP</li>
<li>可在终端页面中对已连接的资产进行操作</li>
</ol>

<h3>资产认证信息</h3>
<p>通过堡垒机连接资产时，系统使用资产表中存储的认证信息：</p>
<ul>
<li><b>用户名</b> — 来自资产表的"用户名"字段</li>
<li><b>密码</b> — 来自资产表的"密码"字段</li>
</ul>
<p>请确保资产已正确配置用户名和密码，否则无法连接。</p>
...
<li>通过堡垒机连接资产需要资产已配置用户名和密码</li>
<li>连接状态显示在状态栏，可查看当前连接的资产IP</li>
</ul>
"""
    },
    {
...
<tr><td>远程执行</td><td>环境切换</td><td>切换到远程执行环境，后续节点在远程主机执行，直至遇到本机执行节点</td></tr>
<tr><td>本机执行</td><td>环境切换</td><td>切换回本地执行环境，后续节点在本机执行</td></tr>
</table>
...
<h3>环境切换节点</h3>
<p>环境切换节点用于控制工作流的执行环境：</p>
<ul>
<li><b>远程执行节点</b>：切换到远程执行环境，需要先在主界面连接堡垒机。配置目标主机地址时，可从下拉菜单选择已连接的主机，或手动输入IP地址。系统会自动记忆历史输入的主机地址。</li>
<li><b>本机执行节点</b>：切换回本地执行环境，后续所有动作节点将在本机执行。</li>
</ul>
<p><b>注意</b>：远程执行环境仅支持Bash脚本，Python和SQL脚本需要在本地执行环境运行。如果堡垒机未连接，远程执行节点将执行失败并提示错误。</p>
"""
    },
    {
        "id": "audit",
...
<li>认证成功后，如堡垒机返回服务器列表，会弹出服务器选择对话框</li>
<li>在服务器选择对话框中可以搜索、选择目标服务器，也可以关闭对话框稍后选择</li>
</ol>

<h3>操作步骤 — 选择服务器</h3>
<ol>
<li>认证成功后，如果堡垒机返回了服务器列表，会自动弹出服务器选择对话框</li>
<li>服务器选择对话框为非模态窗口，可以同时操作主界面其他功能</li>
<li>关闭对话框后，可点击状态栏堡垒机状态指示器，选择"选择服务器"重新打开</li>
<li>在对话框中可通过IP或名称搜索服务器，双击行或选中后点击"连接"即可</li>
</ol>
