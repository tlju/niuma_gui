from PyQt6.QtWidgets import (
    ...
    QFrame, QFormLayout, QTabWidget, QSpinBox
)
...
class DataDictsPage(QWidget):
    ...
    def init_ui(self):
        ...
        self.dict_table.setHorizontalHeaderLabels(["ID", "代码", "名称", "描述", "操作"])
        ...
        self.item_table.setHorizontalHeaderLabels(["ID", "项代码", "项名称", "项值", "排序", "操作"])
    ...
    def _populate_dicts(self, dicts):
        ...
        for row, d in enumerate(dicts):
            self.dict_table.setItem(row, 0, QTableWidgetItem(str(d.id)))
            self.dict_table.setItem(row, 1, QTableWidgetItem(d.code or ""))
            self.dict_table.setItem(row, 2, QTableWidgetItem(d.name or ""))
            self.dict_table.setItem(row, 3, QTableWidgetItem(d.description or ""))
            ...
            self.dict_table.setCellWidget(row, 4, btn_widget)
    ...
    def _populate_items(self, items):
        ...
        for row, item in enumerate(items):
            ...
            self.item_table.setItem(row, 3, QTableWidgetItem(item.item_value or ""))
            self.item_table.setItem(row, 4, QTableWidgetItem(str(item.sort_order or 0)))
            ...
            btn_layout.addWidget(edit_btn)
            btn_layout.addWidget(delete_btn)
            self.item_table.setCellWidget(row, 5, btn_widget)
...
class DictItemDialog(QDialog):
    ...
    def init_ui(self):
        ...
        layout.addRow("项值:", self.value_input)
    ...
    def _populate_data(self):
        ...
        self.value_input.setText(self.item.item_value or "")
    ...
    def get_data(self):
        return {
            ...
            "item_value": self.value_input.text(),
